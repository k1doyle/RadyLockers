...
import { formatAdminDate, formatAdminDateTime, formatCurrency, formatStatus, getComboValue } from '@/lib/utils';
...
export default async function LockerDetailPage({
  ...
  const canMoveToPendingReturn = activeAssignment && ['ASSIGNED', 'OVERDUE'].includes(locker.status);
  const canCompleteReturn = latestAssignment?.request_status === 'ASSIGNED' && ['PENDING_RETURN', 'OVERDUE'].includes(locker.status);
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">MPR2 / IT offices exterior area · Outdoor metal combination locker</p>
          </div>
          <div className="flex items-center gap-3">
            <StatusBadge status={locker.status} />
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
              Active combo slot {locker.active_combo_index} · next advance wraps after 5
            </span>
          </div>
        </div>
        ...
        <section className="mt-8 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-500">Admins can edit preset combinations and the tracked active combination index.</p>
                </div>
                  ...
                  assignments.map((assignment) => (
                    <div key={assignment.request_id} className="rounded-2xl border border-slate-200 p-4">
                      ...
                      <div className="mt-3 grid gap-2 text-sm text-slate-600 md:grid-cols-2">
                        ...
                        <p>Returned: {assignment.returned_date ? formatAdminDate(assignment.returned_date) : 'Not yet returned'}</p>
                      </div>
                    </div>
                  ))
                ) : (
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {canMoveToPendingReturn && activeAssignment ? (
                <div className="mt-5 space-y-5 text-sm text-slate-600">
                  <div className="rounded-2xl bg-slate-50 p-4">
                    ...
                    <p className="mt-2">Ends: {activeAssignment.assignment_end_date ? formatAdminDate(activeAssignment.assignment_end_date) : 'Not set'}</p>
                  </div>
              ...
              ) : canCompleteReturn && latestAssignment ? (
                <form action={completeReturn} className="mt-5 space-y-4 rounded-2xl border border-slate-200 p-4 text-sm text-slate-600">
                  ...
                  <input type="hidden" name="locker_id" value={locker.locker_id} />
                  <input type="hidden" name="expected_active_combo_index" value={locker.active_combo_index} />
                  ...
                  <label className="flex items-center gap-3 text-sm text-slate-700">
                    ...
                    Advance to the next preset combination after verifying the locker is empty
                  </label>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-600">
                The app only tracks the active combo index. Staff should advance the physical lock separately, then record the next slot here.
              </p>
              <form action={advanceCombo} className="mt-5 space-y-3">
                <input type="hidden" name="locker_id" value={locker.locker_id} />
                <input type="hidden" name="expected_active_combo_index" value={locker.active_combo_index} />
                <button className="rounded-xl bg-brand-blue px-4 py-3 text-sm font-semibold text-white">Advance combo position</button>
                <p className="text-xs text-slate-500">This cycles 1 → 2 → 3 → 4 → 5 → 1 and rejects stale double submissions.</p>
              </form>
            </div>
                ...
                {auditLogs.length ? (
                  auditLogs.map((log) => (
                    <div key={log.id} className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <p className="mt-2 text-xs uppercase tracking-wide text-slate-400">{formatAdminDateTime(log.created_at)}</p>
                    </div>
                  ))
                ) : (

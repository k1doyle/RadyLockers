      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-brand-navy">Outdoor locker inventory (MPR2 / IT offices)</h1>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
              Review incoming requests, assign available lockers, and keep quarter-end returns moving.
            </p>
          </div>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Available lockers" value={metricMap.get('AVAILABLE') ?? 0} description="Ready to assign" />
          <MetricCard label="Assigned lockers" value={metricMap.get('ASSIGNED') ?? 0} description="Currently in use" />
          <MetricCard label="Pending return" value={metricMap.get('PENDING_RETURN') ?? 0} description="Need return check" />
          <MetricCard label="Disabled lockers" value={metricMap.get('DISABLED') ?? 0} description="Out of service" />
        </section>
        ...
        <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-200 pb-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Prioritize the oldest requests and note who may ask to renew.</p>
            </div>
              ...
              requests.map((request) => (
                <div key={request.request_id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 space-y-1 text-sm text-slate-600">
                    <p>{request.program}</p>
                    <p>{request.requested_quarter}</p>
                    <p>Requested {formatAdminDate(request.created_at)}</p>
                    <p>{request.renewal_requested ? 'Possible renewal request' : 'One-quarter request'}</p>
                  </div>
                  <Link href={`/admin/request/${request.request_id}`} className="mt-4 inline-flex text-sm font-medium text-brand-blue">
                    Review request
                  </Link>
                </div>
              ))
        ...
        <section className="mt-6 grid gap-6 xl:grid-cols-[1.8fr_1fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Search by locker, student, email, location, or program.</p>
            </div>
            <form className="mt-5 grid gap-3 lg:grid-cols-6 lg:items-end">
              <input
                name="search"
                defaultValue={typeof params.search === 'string' ? params.search : ''}
                placeholder="Search lockers or students"
              ...
              <div className="flex gap-3 lg:justify-end">
                ...
                <Link href="/admin" className="rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700">Clear</Link>
              </div>
            </form>
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {lockers.length ? lockers.map((locker) => (
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="border-b border-slate-200 pb-5">
              <h2 className="text-xl font-semibold text-brand-navy">Add locker</h2>
              <p className="mt-1 text-sm text-slate-500">Use this when a new outdoor metal locker is ready for staff assignment.</p>
            </div>
            <form action={createLocker} className="mt-5 space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <input name="locker_number" placeholder="Locker number" required className="rounded-xl border border-slate-300 px-4 py-3 text-sm" />
                <input name="location" placeholder="Location" required className="rounded-xl border border-slate-300 px-4 py-3 text-sm" />
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              <textarea name="notes" placeholder="Operational notes" className="min-h-24 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm" />
              <textarea name="disabled_reason" placeholder="Disabled reason, if needed" className="min-h-20 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm" />
              <button className="w-full rounded-xl bg-brand-navy px-4 py-3 text-sm font-semibold text-white">Save locker</button>
            </form>
          </section>
        </section>
      </div>

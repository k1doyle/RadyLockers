...
const requestSchema = z.object({
  ...
  requested_rental_period: z.enum(['One Academic Quarter', 'One Academic Quarter, with possible renewal request']),
...
export async function submitLockerRequest(formData: FormData) {
  ...
  db.prepare(`
    INSERT INTO assignments (
      student_name, ucsd_email, pid_or_student_id, program, requested_quarter, renewal_requested, request_status,
  ...
  `).run(
    ...
    data.requested_rental_period === 'One Academic Quarter, with possible renewal request' ? 1 : 0,
    data.reason || null,
    now,
    now,
  );
